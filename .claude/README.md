...
1M 레코드 환경에서 다양한 검색 시나리오별 응답 시간을 측정하여 각 솔루션의 검색 성능을 비교 분석함.
...
각 데이터베이스 프레임워크의 검색 성능을 측정하고 파일로 내보냄 (1M 레코드 기준)
...
- 1M 레코드 고정 (데이터 양 변화 없음)

## 검색 테스트 전제 조건

1M 레코드의 fixture 파일이 사전에 준비되어 있어야 함
...
Fixture 파일에서 1M 레코드를 일괄 로드한 후 4가지 검색 시나리오(Equality, Range, Complex, Full-Text)를 실행하여 응답 시간 측정.
...
Fixture 파일 존재 확인: 1M 레코드 파일이 올바른 위치에 있는지 검증
