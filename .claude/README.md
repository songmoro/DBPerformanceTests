...
Result Comparison UI를 통해 저장된 검색 벤치마크 결과를 시각적으로 비교 가능
- 비교 대상: SearchBenchmarkReport (검색 벤치마크 결과만)
- UI 구성: TabView로 Benchmarks 탭과 Comparison 탭 분리
- 주요 기능:
  - 저장된 JSON 파일 선택 (최소 1개, 최대 4개)
  - 시나리오별 응답시간 비교 차트 (막대 그래프)
  - DB별 성능 순위 (1st/2nd/3rd 표시)
  - 환경 정보 및 메타데이터 비교 테이블
