...
[TM-05] Fixture 파일을 단일 스냅샷으로 로드 (1M 레코드 일괄)
[TM-06] 로딩 시간 측정 (포함: 파일 읽기 + 파싱 + DB 저장)
[TM-07] 로딩 완료 후 모든 데이터가 DB에 존재함을 검증 (기대값: 1,000,000)
...
[TM-33] 파일 크기: flat-1m.json ~800MB, relational-1m.json ~1.2GB
[TM-34] 로딩 성능 측정: 파일 읽기 + 파싱 + DB 저장 총 시간(ms)
[TM-35] 로딩 후 데이터 검증: 실제 저장 레코드 수 == 1,000,000
